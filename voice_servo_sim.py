#!/usr/bin/python
# RUN ON LAPTOP - SIMULATION VERSION
"""
Voice-Controlled Visual Servoing System - SIMULATION MODE
All features work exactly the same, but robot connection is simulated
...
class SimulatedCamera:
    """Simulated camera that generates test frames with colored objects"""
    ...
    def __init__(self):
        ...
        # Simulated object positions (pixel coordinates)
        self.objects = {
            'red': {'pos': (320, 240), 'size': 40},  # Gripper center
            'yellow': {'pos': (200, 300), 'size': 30},
            'green': {'pos': (450, 250), 'size': 35},
            'blue': {'pos': (320, 350), 'size': 28}
        }
        
        print("📷 [SIM] Camera initialized (640x480)")
    
    def read(self):
        """Generate a simulated camera frame with colored objects"""
        # Create blank frame
        frame = np.ones((self.height, self.width, 3), dtype=np.uint8) * 50  # Dark gray background
        
        # Draw colored circles for each object
        colors_bgr = {
            'red': (0, 0, 255),
            'yellow': (0, 255, 255),
            'green': (0, 255, 0),
        ...
        for color_name, obj in self.objects.items():
            ...
            cv2.circle(frame, pos, size, color_bgr, -1)
            # Add slight border for better detection
            cv2.circle(frame, pos, size, color_bgr, 2)
    ...
    def release(self):
        """Release camera"""
        self.is_open = False
        print("📷 [SIM] Camera released")


class SimulatedRobotServer:
    """Simulated robot server that mimics CartesianServer behavior"""
    
    def __init__(self, ip, port):
        print(f"🔧 [SIM] Connecting to robot at {ip}:{port}...")
        time.sleep(0.5)
        self.x = 3.75  # Start at center
        self.y = 3.5
        self.z = 0.0
        self.gripper_open = False
        print(f"🔧 [SIM] Robot initialized at ({self.x:.2f}, {self.y:.2f}, {self.z:.2f})")
    ...
    def sendMove(self, x, y, z, queue):
        ...
        distance = np.sqrt((x - old_pos[0])**2 + (y - old_pos[1])**2 + (z - old_pos[2])**2)
        sim_time = min(0.1, distance * 0.02)  # Faster simulation
        time.sleep(sim_time)
        
        queue.put("MOVE_COMPLETE")
        print(f"🔧 [SIM] Moved to ({self.x:.2f}, {self.y:.2f}, {self.z:.2f})")
    ...
    def sendGripperOpen(self, queue):
        ...
        queue.put("GRIPPER_OPEN")
        print(f"🔧 [SIM] Gripper OPENED")
    
    def sendGripperClose(self, queue):
        """Simulate closing gripper"""
        time.sleep(0.2)
        self.gripper_open = False
        queue.put("GRIPPER_CLOSED")
        print(f"🔧 [SIM] Gripper CLOSED")
    
    def sendExit(self):
        """Simulate disconnect"""
        print(f"🔧 [SIM] Disconnected from robot")
...
def scan_objects(camera, color_ranges):
    ...
    cv2.imshow("Lil B Vision [SIMULATION]", display_frame)
    cv2.waitKey(2000)
    ...
    try:
        while iteration < MAX_ITERATIONS and not should_exit:
            ...
            if paused:
                ...
                cv2.imshow("Lil B Vision [SIMULATION]", output)
                cv2.waitKey(30)
                continue
            ...
            cv2.putText(output, f"Iteration: {iteration}", (10, 60),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            cv2.putText(output, "[SIMULATION MODE]", (10, height - 10),
...
def main():
    ...
    print("="*60)
    print("🎙️  LIL B - VOICE CONTROLLED ROBOT [SIMULATION MODE]")
    print("="*60)
    
    print("\n🤖 Connecting to robot (SIMULATED)...")
    server = SimulatedRobotServer("169.254.207.188", 9999)  # Using simulated server
    print("✓ Robot connected (SIMULATED)!")
    
    print("📷 Opening camera (SIMULATED)...")
    camera = SimulatedCamera()  # Using simulated camera
    ...
    print("⚠️  SIMULATION MODE - No physical robot needed!")
...
if __name__ == "__main__":
    ...
    print("  1. ✓ Robot connection - SIMULATED (no physical robot needed)")
    print("  2. ✓ Camera - SIMULATED (shows test objects)")
    print("  3. ✓ Colored objects - SIMULATED")
    print("  4. Microphone is working (for voice commands)\n")
    
    input("Press Enter to start Lil B in full simulation mode...")
    main()
